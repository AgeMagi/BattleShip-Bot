...
void Bot::fireShot() {
	...
	if (!shotLocation.valid()) {
		...
		if (!validCrossCheck()) {
			while(map[shotLocation.x][shotLocation.y].damaged || map[shotLocation.x][shotLocation.y].missed) {
				shotLocation.x = rand()%size;
				shotLocation.y = rand()%size;
			}
		}
...
void Bot::placeShips() {
	...
	ofs << "Carrier 9 2 West\n";
	ofs << "Battleship 12 6 North\n";
	ofs << "Cruiser 1 6 North\n";
	ofs << "Submarine 2 9 South\n";
	ofs << "Destroyer 4 2 North\n";
}
...
bool Bot::validCrossCheck() {
	for(int x = 0; x < size; x++) {
		for(int y = 0; y < size; y++) {
			if ((x + y)%2 == 0) {
				if (!map[x][y].damaged || !map[x][y].missed) {
					return true;
				}
			}
		}
